 ...
 * 目标：实现 8 大核心链表与本地 .txt 文件的双向同步 [cite: 30, 47]
 * ========================================== */

// 统一总控：程序启动时加载所有数据，退出时执行静默保存 [cite: 30]
...
void load_ambulances(const char* filepath); // 新增：救护车队信息加载
...
void save_ambulances(const char* filepath); // 新增：救护车状态与位置保存
...
AmbulanceNode* find_ambulance_by_id(const char* id); // 新增

// --- 复杂业务逻辑检索 ---

// 模糊搜索：支持通过药品名或别名进行关键词检索 [cite: 46]
void search_medicine_by_keyword(const char* keyword);

// 资源自动分配：查找指定科室及病房类型(如 ICU)的空闲床位 [cite: 63, 64]
BedNode* find_available_bed(const char* dept_id, WardType type);

// 核心流水记录生成：创建 RecordNode，含自动挂号排队号与医保核算 [cite: 14, 15]
