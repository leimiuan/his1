...
/**
 * 患者病房转换与转科申请
 * 支持 7 类病房切换：ICU、单人、双人、三人、陪护、疗养房
 */
...
/**
 * 药品库存检索
 * 允许医生在开方前查询特定药品的余量与单价
 */
void query_medicine_inventory();


/* ==========================================
 * --- 6. 应急与重症联动查询模块 (【绝对指令新增】) ---
 * 目标：为医生提供手术室与院前急救资源的透明度
 * ========================================== */

// (注：以下功能已通过静态嵌入的方式织入 doctor_subsystem 主菜单中，
// 核心业务逻辑直接从 common.h 调用底层全局数据，此处无需独立暴漏业务函数，仅作功能升级说明)

/**
 * @brief 全院手术室实时状态追踪 (急救联动)
 * 功能描述：在 doctor_subsystem 的菜单 9 中触发。
 * 允许接诊医生实时查看 OR-01 等手术室的空闲、手术或消毒状态，为抢救赢得时间。
 */

/**
 * @brief 查看 120 救护车历史出勤表 (急诊参考)
 * 功能描述：在 doctor_subsystem 的菜单 10 中触发。
 * 允许医生查阅最新救护车派遣日志流水，了解即将送达的患者来源或运力情况。
 */
