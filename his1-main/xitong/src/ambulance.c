...
// 【跨模块联动声明】：引入挂号生成引擎，用于救护车回场时自动拉起急诊通道
...
void ambulance_subsystem() {
    ...
    while (choice != 0) {
        ...
        switch (choice) {
            ...
            case 3: {
                // ==========================================
                // 业务 3：回场登记 - 结束任务并释放车辆资源 (+ 急救绿色通道联动)
                // ==========================================
                printf("请输入回场车辆编号: ");
                char vid[MAX_ID_LEN]; 
                safe_get_string(vid, MAX_ID_LEN);
                ...
                if (target && target->status == AMB_DISPATCHED) {
                    ...
                    if (safe_get_int() == 1) {
                        ...
                        // 强制拉起 RECORD_EMERGENCY (急诊) 单据，直接推入底层链表
                        // 急诊诊费定为 50.0 元，queue_number 置 0 (急诊不排号，绝对优先)
