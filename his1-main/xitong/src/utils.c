 ...
 * 使用 fgets 替代危险的 scanf，能有效防止缓冲区溢出攻击 (Buffer Overflow)。
...
int safe_get_int() {
    ...
    while (1) {
        ...
        // 3. errno != ERANGE : 确保数值没有发生系统级溢出
        // 4. >= INT_MIN && <= INT_MAX : 确保 long 能安全向下转型为 int
        if (end_ptr != input &&
...
double safe_get_double() {
    ...
    while (1) {
        ...
        errno = 0;
        // 使用 strtod 安全转换为 double
        value = strtod(input, &end_ptr);
        
        // 同样的严密校验逻辑，防止 "12.5abc" 或极端大数溢出
