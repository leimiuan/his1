...
static const char* record_type_text(RecordType type) {
    switch (type) {
        ...
        case RECORD_EMERGENCY:    return COLOR_RED "急诊" COLOR_RESET;
        default: return "未知";
    }
}
...
static const char* record_status_text(int status) {
    ...
    if (status == -1) return COLOR_RED "已撤销" COLOR_RESET;
    return "未知";
}
...
// 移除 static，与 admin.h 匹配 (同时修复函数名为 query_records_by_department)
...
// 移除 static，与 admin.h 匹配 (修正为 query_records_by_patient_keyword)
...
void admin_subsystem() {
    ...
    while (choice != 0) {
        ...
        switch (choice) {
            ...
            case 4: manage_ambulances(); break;
