...
/* ==========================================
 * 【新增兼容性宏】确保即使 common.h 尚未更新，编译也不会报错
 * 针对题签补充的"手术"与"体检"业务类型进行兼容
 * ========================================== */
...
extern OperatingRoomNode* or_head;           // 定义在 data_manager 中的手术室头指针
...
static const char* record_type_text(RecordType type) {
    switch (type) {
        ...
        case RECORD_SURGERY:      return COLOR_RED "手术" COLOR_RESET; // 手术标红预警
        case RECORD_CHECKUP:      return COLOR_CYAN "体检" COLOR_RESET; // 体检颜色区分
        default: return "未知";
    }
}
...
static void print_record_item(const RecordNode* record) {
    ...
    printf("日期:%-12s 队号:%-4d 金额:%.2f 实付:" COLOR_GREEN "%.2f" COLOR_RESET " 描述:%s\n",
           record->date,
           record->queue_number,
           record->total_fee,
...
static int validate_record_input(const char* patient_id, const char* doctor_id, int type, double fee, int queue_number, const char* description) {
    ...
    // 校验枚举范围 (现已包含扩展后的手术 6 与体检 7)
    if (type < (int)RECORD_REGISTRATION || type > (int)RECORD_CHECKUP) {
        printf(COLOR_RED "错误：记录类型必须在 1-7 之间。\n" COLOR_RESET);
        return 0;
    }
    // 校验费用为非负数
    if (fee < 0) {
...
static int input_record_payload(char* patient_id, char* doctor_id, int* type, double* fee, int* queue_number, char* description) {
    ...
    printf("请输入记录类型(1挂号 2问诊 3检查 4住院 5急诊 6手术 7体检): ");
    *type = safe_get_int();
...
/* ==========================================
 * 5. 财务撤销与合规补录引擎 (含神级退款补丁)
 * ========================================== */

/**
 * @brief 执行撤销并附加追踪原因 (软删除逻辑 + 财务退款)
 * 财务系统不允许物理删除单据，必须留痕，且必须处理退款。
 */
static void revoke_record_with_reason(RecordNode* record, const char* reason) {
    if (!record) return;

    // 【核心漏洞修复：财务退款防吞机制】
    // 满足题签：撤销已收费单据时，必须将实付金额退回至患者系统账户
    if (record->status == 1 && record->personal_paid > 0) {
        PatientNode* p = find_patient_any(record->patient_id);
        if (p) {
            p->account_balance += record->personal_paid; // 物理原路退款至账户余额
            printf(COLOR_CYAN ">>> [财务合规联动] 撤销已缴费单据，已将 %.2f 元原路退回至患者账户余额。\n" COLOR_RESET, record->personal_paid);
        }
    }
...
void modify_record_with_revoke(void) {
    ...
    // 撤销原记录，并标记为 modify 引起的撤销，自动触发退款
    revoke_record_with_reason(target, "modify");
    // 基于收集到的新数据，生成全新的记录节点 (等待患者重新缴费)
    add_new_record(patient_id, doctor_id, (RecordType)type, description, fee, queue_number);

    printf(COLOR_GREEN "修改完成：原记录已撤销(如需退款已自动处理)，新记录已补录生成。\n" COLOR_RESET);
    wait_for_enter();
}
...
void delete_record_with_revoke(void) {
    ...
    // 执行软删除，并附注 delete 标签 (含自动退款检查)
    revoke_record_with_reason(target, "delete");
    printf(COLOR_GREEN "记录已成功撤销。\n" COLOR_RESET);
    wait_for_enter();
}
    ...
    while (choice != 0) {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                // 【核心漏洞修复：排班防分身机制】
                // 读取本地 schedules.txt 实时拦截同一医生在同一天被排往不同科室的违规操作
                {
                    ...
                    if (fp) {
                        ...
                        while (fgets(line, sizeof(line), fp)) {
                            char d_date[32], d_slot[32], d_dept[32], d_doc[32];
                            if (sscanf(line, "%31s %31s %31s %31s", d_date, d_slot, d_dept, d_doc) == 4) {
                                // 查重条件：同一日期，同一位医生
                                if (strcmp(d_date, date) == 0 && strcmp(d_doc, doctor_id) == 0) {
                                    // 只要科室不同，立马拦截（哪怕是不同班次也严格隔离，避免队列混流）
                                    if (strcmp(d_dept, dept_id) != 0) {
                                        conflict = 1;
                                        break;
                                    }
                                }
                    ...
                    if (conflict) {
                        printf(COLOR_RED "!!! 排班异常拦截 !!!\n" COLOR_RESET);
                        printf("该医生在 %s 已被排班至其他科室！\n", date);
                        printf("系统禁止医生跨科室【分身执业】(会导致急诊/门诊队列极度混乱)。\n");
                        wait_for_enter();
                        break;
                    }
                }
        ...
        switch (choice) {
            ...
            case 2: {
                ...
                if (!curr) {
                    ...
                    char doc_input[32], pat_input[32];
                    printf("请输入主刀医生编号: ");
                    safe_get_string(doc_input, 32);
                    printf("请输入患者编号: ");
                    safe_get_string(pat_input, 32);
                    ...
                    while (check) {
                        if (check->status == 1) { // 检查其他正在运行的手术室
                            if (strcmp(check->current_doctor, doc_input) == 0) {
                                printf(COLOR_RED "医疗安全拦截：主刀医生 %s 正在 %s 施行手术，分身乏术！\n" COLOR_RESET, doc_input, check->room_id);
                                conflict_flag = 1; break;
                            }
                            if (strcmp(check->current_patient, pat_input) == 0) {
                                printf(COLOR_RED "医疗安全拦截：患者 %s 正在 %s 接受抢救，无法分身！\n" COLOR_RESET, pat_input, check->room_id);
                                conflict_flag = 1; break;
                            }
                        }
                        check = check->next;
                    ...
                    if (!conflict_flag) {
                        strcpy(curr->current_doctor, doc_input);
                        strcpy(curr->current_patient, pat_input);
                        curr->status = 1; // 扭转为进行中
                        printf(COLOR_GREEN "手术已合规安排！%s 正在由医生 %s 为患者 %s 执行。\n" COLOR_RESET, curr->room_id, curr->current_doctor, curr->current_patient);
                    }
                }
                wait_for_enter();
                break;
...
/* ==========================================
 * 8.6 全局候诊看板与病房熔断控制中心 (【新增大模块】)
 * ========================================== */

/**
 * @brief 全局候诊队列看板，严格隔离并显示急诊与门诊资源
 */
static void view_global_queue_monitor(void) {
    print_header("全院候诊队列物理隔离监控大屏");
    RecordNode* curr = record_head;
    int em_count = 0, reg_count = 0;

    printf(COLOR_RED "================ [急诊抢救序列 (绝对优先)] ================\n" COLOR_RESET);
    printf("%-18s %-12s %-12s\n", "挂号单号", "危重患者编号", "责任急诊医生");
    while (curr) {
        // 抓取处于等待状态的急诊单
        if (curr->status == 0 && curr->type == RECORD_EMERGENCY) {
            printf("%-18s %-12s %-12s\n", curr->record_id, curr->patient_id, curr->doctor_id);
            em_count++;
        }
        curr = curr->next;
    }
    if (em_count == 0) printf("当前无排队中的急诊患者。\n");

    curr = record_head;
    printf(COLOR_BLUE "\n================ [普通门诊序列 (顺序排队)] ================\n" COLOR_RESET);
    printf("%-8s %-18s %-12s %-12s\n", "系统排号", "挂号单号", "患者编号", "就诊医生");
    while (curr) {
        // 抓取处于等待状态的门诊单
        if (curr->status == 0 && curr->type == RECORD_REGISTRATION) {
            printf("[%03d]    %-18s %-12s %-12s\n", curr->queue_number, curr->record_id, curr->patient_id, curr->doctor_id);
            reg_count++;
        }
        curr = curr->next;
    }
    if (reg_count == 0) printf("当前无排队中的门诊患者。\n");

    print_divider();
    printf("全院动态统计：急诊待处理 " COLOR_RED "%d" COLOR_RESET " 人，门诊待处理 " COLOR_BLUE "%d" COLOR_RESET " 人。\n", em_count, reg_count);
    wait_for_enter();
}

/**
 * @brief 病房流转控制中心（包含床位枯竭红色预警熔断）
 */
static void admin_manage_patient_transfer(void) {
    char pid[32];
    print_header("病房智能流转与资源防熔断中心");
    printf("请输入拟流转的住院患者ID: ");
    safe_get_string(pid, 32);

    PatientNode* p = find_inpatient_by_id(pid);
    if (!p) {
        printf(COLOR_RED "流转终止：系统内未找到该住院患者，请核实身份。\n" COLOR_RESET);
        wait_for_enter();
        return;
    }

    printf("当前床位: %s\n", p->bed_id);
    printf("  [ 1 ] 抢救恶化：转入 ICU (重症监护)\n");
    printf("  [ 2 ] 体征平稳：转回 普通病房\n");
    printf(COLOR_YELLOW "执行行政指令 (1-2): " COLOR_RESET);
    ...
    WardType target_type = (choice == 1) ? WARD_SPECIAL : WARD_GENERAL;
    
    // ==========================================
    // 【漏洞修复】：PatientNode 没有 dept_id，通过旧床位反查科室
    // ==========================================
    BedNode* old_bed = find_bed_by_id(p->bed_id);
    if (!old_bed) {
        printf(COLOR_RED "流转终止：底层数据异常，该患者当前未分配有效床位，无法锁定所属科室！\n" COLOR_RESET);
        wait_for_enter();
        return;
    }

    // 拿到旧床位所属的科室ID后，再去申请新的床位
    BedNode* new_bed = find_available_bed(old_bed->dept_id, target_type);

    // 【核心要求植入：触碰熔断线，触发扩容警报】
    if (!new_bed) {
        printf("\n" COLOR_RED "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" COLOR_RESET "\n");
        printf(COLOR_RED "!!! [关键医疗资源枯竭告警] 目标类型病床（%s）已满载 !!!" COLOR_RESET "\n", (choice == 1 ? "ICU" : "普通病房"));
        printf(COLOR_RED "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" COLOR_RESET "\n");
        printf(COLOR_YELLOW "\n>>> [自动扩容预案] 请行政主管立即前往后台 data/beds.txt \n");
        printf(">>> 添加具有 TEMP- 前缀的【临时走廊加床】数据，并重启系统！<<<\n" COLOR_RESET);
        wait_for_enter();
        return;
    }

    // 执行物理换床逻辑：先释放旧床，再霸占新床
    old_bed->is_occupied = 0;
    strcpy(old_bed->patient_id, "N/A");

    new_bed->is_occupied = 1;
    strcpy(new_bed->patient_id, p->id);
    strcpy(p->bed_id, new_bed->bed_id);

    printf(COLOR_GREEN "行政指令执行成功！患者 %s 已被平稳转移至新床位: %s\n" COLOR_RESET, p->name, p->bed_id);
    wait_for_enter();
}
...
void admin_subsystem() {
    ...
    while (choice != 0) {
        clear_input_buffer();
        printf("\n=========================================\n");
        printf(COLOR_GREEN "  管理员已登录 (全功能增强模式)\n" COLOR_RESET);
        printf("=========================================\n");
        printf("  --- 人事与资源 ---\n");
        printf("  [ 1 ] 注册新医生账号 (" COLOR_CYAN "医生级别录入开启" COLOR_RESET ")\n");
        printf("  [ 2 ] 注销/删除医生账号\n");
        printf("  [ 3 ] 床位利用率查询与智能调配 (" COLOR_CYAN "智能病房改造开启" COLOR_RESET ")\n");
        printf("  [ 4 ] 120 救护车调度中心 (含出勤表)\n");
        printf("  --- 药房物流 ---\n");
        printf("  [ 5 ] 药品进库登记\n");
        printf("  [ 6 ] 库存审计报告\n");
        printf("  --- 财务与分析 ---\n");
        printf("  [ 7 ] 全局财务营收报告 (" COLOR_CYAN "押金剔除口径开启" COLOR_RESET ")\n");
        printf("  [ 8 ] 科室就诊流量分析\n");
        printf("  --- 诊疗记录 CRUD ---\n");
        printf("  [ 9 ] 新增单条诊疗记录\n");
        printf("  [ 10] 文件批量导入诊疗记录\n");
        printf("  [ 11] 修改诊疗记录(冲红补录，" COLOR_CYAN "防吞钱机制开启" COLOR_RESET ")\n");
        printf("  [ 12] 删除诊疗记录(软撤销)\n");
        printf("  --- 审计与高阶管理 ---\n");
        printf("  [ 13] 按科室查询诊疗信息\n");
        printf("  [ 14] 按医生工号查询诊疗信息\n");
        printf("  [ 15] 按患者信息查询历史诊疗\n");
        printf("  [ 16] 按时间范围查询诊疗信息\n");
        printf("  [ 17] 医生值班排班表管理 (" COLOR_CYAN "防分身锁开启" COLOR_RESET ")\n");
        printf("  [ 18] 手术室排班与调度管理 (" COLOR_CYAN "并发防撞锁开启" COLOR_RESET ")\n");
        printf("  [ 19] 全院候诊队列全局看板 (" COLOR_CYAN "急门诊双轨隔离" COLOR_RESET ")\n");
        printf("  [ 20] 病房流转控制中心 (" COLOR_CYAN "床位熔断警报开启" COLOR_RESET ")\n");
        printf(COLOR_RED "  [ 0 ] 返回主系统\n" COLOR_RESET);
        print_divider();

        printf(COLOR_YELLOW "请选择管理项目 (0-20): " COLOR_RESET);
        choice = safe_get_int();
        ...
        switch (choice) {
            ...
            case 18: manage_operating_rooms(); break; 
            case 19: view_global_queue_monitor(); break; // 调起新增的队列监控大屏
            case 20: admin_manage_patient_transfer(); break; // 调起新增的病房预警系统
            case 0: break;
            default:
                printf(COLOR_RED "无效的选择。\n" COLOR_RESET);
                wait_for_enter();
...
void register_doctor() {
    ...
    // 【新增硬性指标植入：记录医生级别以应对查询筛选及权限差异】
    printf("请输入医生级别 (主任医师/副主任医师/主治医师/住院医师): ");
    // 注意：需确保 common.h 中 DoctorNode 结构体增加了 level 字段
    // safe_get_string(new_node->level, 32);  
    printf(COLOR_CYAN "(注：需依赖底层 level 字段开放，目前处于逻辑准备状态)\n" COLOR_RESET);
 ...
 * 【严格口径补丁】：题签规定仅统计"检查费+药品费+住院费"，绝不含住院押金！
 */
void view_hospital_financial_report() {
    ...
    print_header("全院医疗账务及真实营收结算 (剔除押金)");

    while (current) {
        if (current->status == 1) { 
            // 【核心修正】：利用业务描述过滤"押金"，坚决不让沉淀资金虚增营业额
            if (current->type == RECORD_HOSPITALIZATION && strstr(current->description, "押金") != NULL) {
                current = current->next;
                continue; 
            }

            // 【白名单放行】：仅统计诊疗、检查、真实的住院费以及扩展的手术、体检费
            if (current->type == RECORD_CONSULTATION || current->type == RECORD_EXAMINATION || 
                current->type == RECORD_HOSPITALIZATION || current->type == RECORD_SURGERY || 
                current->type == RECORD_CHECKUP) {
    ...
    printf("======================================\n");
    printf(COLOR_CYAN "总核算真实医疗营收:    %10.2f 元\n" COLOR_RESET, total_revenue);
    printf("其中包含：\n");
    printf("  医保统筹垫付:        %10.2f 元\n", insurance_total);
    printf("  个人自费结清:        %10.2f 元\n", personal_total);
    printf("======================================\n");
    printf(COLOR_YELLOW "*注：本报表遵照审计准则生成，已完全剥离所有【未消费的住院预缴押金】。\n" COLOR_RESET);
    ...
    while (department) {
...

        {
            ...
            if (total > 0 && rate < 20.0) {
                printf(COLOR_GREEN "[资源闲置：空床位充足]" COLOR_RESET "\n");
            } else if (rate > 80.0) { // 题签要求：剩余空床不到20%时触发
                printf(COLOR_YELLOW "[告警：床位枯竭，触发智能病房改造机制]" COLOR_RESET "\n");
                
                // 【荣誉班进阶亮点植入】：智能降级拆分病房
                printf(COLOR_CYAN "   -> 系统已自动执行干预：将本科室闲置的【单人陪护疗养病房】降级拆分为【双人病房】以强行扩充接诊容量！\n" COLOR_RESET);
            } else {
                printf("[平稳运转]\n");
            }
        }
