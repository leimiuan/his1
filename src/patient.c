...
// 并在“呼叫 120 救护车”和“排队状态”模块，打通了急救绿色通道（Score
// 999）的闭环提示。
...
static int is_registration_record(const RecordNode* record) {
  return record && (record->type == RECORD_REGISTRATION ||
                    record->type == RECORD_EMERGENCY);
}
...
static int is_waiting_registration(const RecordNode* record) {
  return is_registration_record(record) && record->status == 1 &&
...
static void ensure_inpatient_cost_records(PatientNode* me) {
  ...
  if (!has_positive_admission_charge(me->id)) {
    add_new_record(me->id, "SYS_BILL", RECORD_HOSPITALIZATION, "住院办理费",
                   50.0, 0);
  }
  ...
  for (time_t t = start_time; t <= end_time; t += 24 * 3600) {
    ...
    snprintf(desc, sizeof(desc), "BEDFEE|%s|BED:%s", day_text,
             (me->bed_id[0] ? me->bed_id : "N/A"));
    add_new_record(me->id, "SYS_BILL", RECORD_HOSPITALIZATION, desc, daily_fee,
                   0);
  }
}
...
void patient_pay_bill(PatientNode* me) {
  ...
  while (curr) {
    if (strcmp(curr->patient_id, me->id) == 0 && curr->status == 0) {
      printf("%-18s %-14d %-10.2f %-10.2f\n", curr->record_id, (int)curr->type,
             curr->total_fee, curr->personal_paid);
      count++;
    }
    curr = curr->next;
  }
  ...
  if (!record || strcmp(record->patient_id, me->id) != 0 ||
      record->status != 0) {
  ...
  while (curr) {
    if (strcmp(curr->patient_id, patient_id) == 0) {
      printf("[%s] %s | 类型=%d | 费用=%.2f | 状态=%d\n", curr->record_id,
             curr->date, (int)curr->type, curr->total_fee, curr->status);
      printf("  描述: %s\n", curr->description);
      count++;
    }
    curr = curr->next;
...
void check_queue_status(PatientNode* patient) {
  ...
  // 3. 计算前方排队人数
  DoctorNode* waiting_doctor = find_doctor_by_id(waiting->doctor_id);
  if (!waiting_doctor) {
    printf("错误：无法找到医生信息。\n");
    wait_for_enter();
    return;
  ...
  curr = record_head;
  while (curr) {
    DoctorNode* curr_doctor = find_doctor_by_id(curr->doctor_id);
    if (curr_doctor &&
        strcmp(curr_doctor->dept_id, waiting_doctor->dept_id) == 0 &&
        (curr->type == RECORD_REGISTRATION || curr->type == RECORD_EMERGENCY) &&
        strstr(curr->description, "|CALLED") == NULL) {
      // 只有同为急诊（或门诊已缴费）的才算在有效队列内
      int is_valid_queue =
          (curr->type == RECORD_EMERGENCY) || (curr->status == 1);
  ...
  // 4. UI 渲染与特权展示
  printf("就诊类型: %s\n", waiting->type == RECORD_EMERGENCY ? COLOR_RED
                               "急诊 (绿色通道)" COLOR_RESET
                                                             : "普通门诊");
...
void patient_self_register(PatientNode* me) {
  ...
  while (1) {
    ...
    if (reg_type == 1 &&
        !is_doctor_on_duty_today_by_slot(target_doctor->id, "门诊")) {
      printf(COLOR_RED
             "输入无效：门诊挂号不能选择急诊排班医生。\n" COLOR_RESET);
    ...
    if (reg_type == 2 &&
        !is_doctor_on_duty_today_by_slot(target_doctor->id, "急诊")) {
      printf(COLOR_RED
             "输入无效：急诊挂号不能选择门诊排班医生。\n" COLOR_RESET);
        ...
        if (strcmp(doc->dept_id, target_doctor->dept_id) == 0) {
          ...
          for (RecordNode* curr = record_head; curr; curr = curr->next) {
            if (is_registration_record(curr) &&
                ...
                strcmp(curr->doctor_id, doc->id) == 0) {
              count++;
            }
          }
          if (count < MAX_DOCTOR_DAILY_TICKETS) {
            printf("- %s（%s），剩余 %d 个号\n", doc->name, doc->id,
                   MAX_DOCTOR_DAILY_TICKETS - count);
          }
        }
      }
  ...
  if (reg_type == 2) {  // 2 代表急诊挂号
    ...
    if (triage_level == 1)
      base_score = 10;  // 1级濒危：11, 12, 13...排在最前！
    else if (triage_level == 2)
      base_score = 100;  // 2级危重：101, 102, 103...
    else if (triage_level == 3)
      base_score = 500;  // 3级急症：501, 502, 503...
    else
      base_score = 900;  // 4级非急症：901, 902...排在最后！
    ...
    while (curr) {
      ...
      if (strcmp(curr->doctor_id, target_doctor->id) == 0 &&
          strncmp(curr->date, today, 10) == 0) {
        if (curr->queue_number > base_score &&
            curr->queue_number <= base_score + 99) {
  ...
  // 调用底层核心工厂，排队逻辑由 data_manager 托管 (注意这里把 0 换成了
  // final_queue_num)
  add_new_record(me->id, target_doctor->id,
                 reg_type == 2 ? RECORD_EMERGENCY : RECORD_REGISTRATION,
                 reg_type == 2 ? "急诊挂号" : "门诊挂号", fee, final_queue_num);
...
void call_ambulance_service(PatientNode* me) {
  ...
  printf(COLOR_CYAN
         "【系统提示】120指挥中心已受理！您到达医院后将自动接入绿色抢救通道。"
         "\n" COLOR_RESET);
...
void patient_dashboard(PatientNode* me) {
  ...
  while (choice != 0) {
    ...
    switch (choice) {
      case 1:
        view_personal_info(me);
        break;
      case 2:
        patient_self_register(me);
        break;
      case 3:
        check_queue_status(me);
        break;
      case 4:
        patient_pay_bill(me);
        break;
      case 5:
        ...
        break;
      case 6:
        call_ambulance_service(me);
        break;
      case 7:
        view_duty_schedule(NULL);
        break;
      case 8:
        query_medical_records(me->id);
        break;
      case 9:
