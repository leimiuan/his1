...
#define MAX_CLINIC_DAILY_TICKETS 500
#define MAX_DOCTOR_DAILY_TICKETS 20
#define MAX_PATIENT_DAILY_TICKETS 5
#endif
...
PatientNode* patient_login(const char* input_id) {
    PatientNode* patient = find_outpatient_by_id(input_id);
...
void view_personal_info(PatientNode* patient) {
    ...
    while (room) {
        if (room->status == 1 && strcmp(room->current_patient, patient->id) == 0) {
            printf(COLOR_RED "手术状态: 正在手术室 %s 手术中\n" COLOR_RESET, room->room_id);
            wait_for_enter();
            return;
        }
        room = room->next;
...
void check_queue_status(PatientNode* patient) {
    ...
    while (curr) {
        if (strcmp(curr->patient_id, patient->id) == 0 && is_registration_record(curr)) {
            if (is_waiting_registration(curr)) {
                waiting = curr;
                break;
            }
            if (!unpaid && curr->status == 0) {
                unpaid = curr;
            }
        }
        curr = curr->next;
    ...
    if (!waiting) {
        if (unpaid) {
            printf(COLOR_YELLOW "你有一条挂号记录尚未缴费。\n" COLOR_RESET);
            printf("记录编号: %s\n", unpaid->record_id);
            wait_for_enter();
            return;
        }
    ...
    while (curr) {
        if (strcmp(curr->doctor_id, waiting->doctor_id) == 0 &&
            is_waiting_registration(curr) &&
            curr->queue_number < waiting->queue_number) {
            ahead++;
        }
        curr = curr->next;
    }

    printf("就诊类型: %s\n", waiting->type == RECORD_EMERGENCY ? "急诊" : "门诊");
...
void patient_self_register(PatientNode* me) {
    ...
    printf(COLOR_GREEN "挂号成功，请先完成缴费后再等待叫号。\n" COLOR_RESET);
    wait_for_enter();
}
...
void register_new_patient() {
    ...
    print_header("挂号新患者");
    snprintf(patient->id, sizeof(patient->id), "P%lld", (long long)time(NULL));
    printf("姓名: ");
