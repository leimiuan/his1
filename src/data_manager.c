...
// 启动自愈：给自动补齐的床位推导默认日费
static double default_daily_fee_by_type(WardType type) {
  switch (type) {
    case WARD_SPECIAL:
      return 1200.0;
    case WARD_ESCORTED:
...
// 根据患者责任医生优先推断科室，其次按床号段推断 (如 B1022 -> DEPT01)
static void infer_dept_for_bed_from_patient(const PatientNode* patient,
                                            char* dept_id,
                                            size_t dept_id_size) {
  int num = 0;
  DoctorNode* doctor = NULL;

  if (!dept_id || dept_id_size == 0) return;
  dept_id[0] = '\0';
  if (!patient) {
    safe_copy_text(dept_id, dept_id_size, "DEPT01");
    return;
  }

  doctor = find_doctor_by_id(patient->attending_doctor_id);
  if (doctor && !is_blank_text(doctor->dept_id)) {
    safe_copy_text(dept_id, dept_id_size, doctor->dept_id);
    return;
  }

  doctor = find_doctor_by_id(patient->resident_doctor_id);
  if (doctor && !is_blank_text(doctor->dept_id)) {
    safe_copy_text(dept_id, dept_id_size, doctor->dept_id);
    return;
  }

  if (sscanf(patient->bed_id, "%*[^0-9]%d", &num) == 1) {
    int dept_no = num / 1000;
    if (dept_no >= 1 && dept_no <= 99) {
      snprintf(dept_id, dept_id_size, "DEPT%02d", dept_no);
      return;
    }
  }
...
// 启动时重建床位占用快照：
// 1) 先清空床位占用状态；2) 依据住院患者重新上锁；3) 若床位缺失则自动补齐节点。
static void reconcile_bed_snapshot_with_inpatients(void) {
  BedNode* bed = bed_head;
  PatientNode* patient = inpatient_head;

  // 先将床位账回到“全空”快照，再按住院患者回填
  while (bed) {
    if (bed->type == 0) {
      bed->type = infer_bed_type_from_snapshot(bed->bed_id, bed->daily_fee);
    }
    bed->is_occupied = 0;
    safe_copy_text(bed->patient_id, sizeof(bed->patient_id), "N/A");
    bed = bed->next;
  }

  while (patient) {
    BedNode* mapped_bed = NULL;

    if (patient->type != 'I' || is_blank_text(patient->bed_id) ||
        strcmp(patient->bed_id, "N/A") == 0) {
      patient = patient->next;
      continue;
    }

    mapped_bed = find_bed_by_id(patient->bed_id);
    if (!mapped_bed) {
      char inferred_dept[MAX_ID_LEN] = {0};
      WardType inferred_type =
          infer_bed_type_from_snapshot(patient->bed_id, 0.0);

      mapped_bed = calloc(1, sizeof(BedNode));
      if (!mapped_bed) {
        patient = patient->next;
        continue;
      }

      safe_copy_text(mapped_bed->bed_id, sizeof(mapped_bed->bed_id),
                     patient->bed_id);
      infer_dept_for_bed_from_patient(patient, inferred_dept,
                                      sizeof(inferred_dept));
      safe_copy_text(mapped_bed->dept_id, sizeof(mapped_bed->dept_id),
                     inferred_dept);
      mapped_bed->type = inferred_type;
      mapped_bed->daily_fee = default_daily_fee_by_type(inferred_type);
      mapped_bed->is_occupied = 0;
      safe_copy_text(mapped_bed->patient_id, sizeof(mapped_bed->patient_id),
                     "N/A");

      mapped_bed->next = bed_head;
      bed_head = mapped_bed;
    }

    // 若存在脏数据导致同床多人，保留先占用者，避免被后续错误覆盖
    if (mapped_bed->is_occupied == 0 ||
        strcmp(mapped_bed->patient_id, patient->id) == 0) {
      mapped_bed->is_occupied = 1;
      safe_copy_text(mapped_bed->patient_id, sizeof(mapped_bed->patient_id),
                     patient->id);
    }
    ...
    if (type == RECORD_REGISTRATION || type == RECORD_EMERGENCY ||
      ...
      while (curr) {
        if ((curr->type == RECORD_REGISTRATION ||
            ...
            // 【核心修复：将原来的 strcmp 替换为 strncmp，只比对前 10
            // 位日期！】
 ...
 * --------------------------------------------------------------------------
 * 核心逻辑：从 /data 目录下的文本数据库读取，以空格或 Tab
 * 为分隔符重建内存链表。
 * ========================================================================== */
...
void load_doctors(const char* filepath) {
  ...
  while (fgets(line, 512, file)) {
    ...
    int parsed =
        sscanf(line, "%31s %127s %31s %d", n->id, n->name, n->dept_id, &priv);
    if (parsed >= 2) {
      if (parsed >= 4) {
...
void load_nurses() {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (sscanf(line, "%[^|]|%[^|]|%[^|]|%d", node->id, node->name,
               node->dept_id, &node->role_level) == 4) {
      if (!nurse_head)
        nurse_head = node;
      else
        tail->next = node;
...
void load_inpatients(const char* filepath) {
  ...
  while (fgets(line, 1024, file)) {
    ...
    int parsed =
        sscanf(line, "%31s %127s %d %15s %c %d %lf %lf %31s %31s %31s %31s",
               n->id, n->name, &n->age, n->gender, &n->type, &ins,
               &n->account_balance, &n->hospital_deposit, n->admission_date,
               n->bed_id, n->attending_doctor_id, n->resident_doctor_id);
...
void load_operating_rooms(const char* filepath) {
  ...
  while (fgets(line, 512, file)) {
    ...
    if (sscanf(line, "%31s\t%d\t%31s\t%31s", n->room_id, &raw_status,
               n->current_patient, n->current_doctor) >= 2) {
      // 手术室状态仅保留二态：0=空闲, 1=手术中。
      // 兼容历史数据中的 2(消毒) 并自动归一化为 0。
      n->status = (raw_status == 1) ? 1 : 0;
      n->next = or_head;
      or_head = n;
    } else
      free(n);
...
/**
 * @brief 持久化保存全院医生人事档案
 * [新增架构逻辑]：在持久化过程中，不仅写入基本身份信息，额外序列化
 * privilege_level 等级到文件行末，确保即使服务器关机重启权限认证策略依旧生效。
 */
void save_doctors(const char* f) {
  FILE* fp = fopen(f, "w");
  if (!fp) return;
  // 打印表头，补充“职称等级”标识
  fprintf(fp, "医生编号\t医生姓名\t科室编号\t职称等级\n");
...
void save_inpatients(const char* f) {
  ...
  // 更新表头，支持双医生列
  fprintf(fp,
          "患者编号\t姓名\t年龄\t性别\t类型\t医保\t余额\t押金\t入院日期\t床位编"
          "号\t主治医生\t管床医生\n");
  for (PatientNode* c = inpatient_head; c; c = c->next) {
    // 容错处理：确保如果医生工号为空时不发生越界
    const char* att_doc =
        (c->attending_doctor_id[0] != '\0') ? c->attending_doctor_id : "N/A";
    const char* res_doc =
        (c->resident_doctor_id[0] != '\0') ? c->resident_doctor_id : "N/A";

    fprintf(fp, "%s\t%s\t%d\t%s\t%c\t%d\t%.2f\t%.2f\t%s\t%s\t%s\t%s\n", c->id,
            c->name, c->age, c->gender, c->type, (int)c->insurance,
            c->account_balance, c->hospital_deposit, c->admission_date,
            c->bed_id, att_doc, res_doc);
  }
  fclose(fp);
}
...
void save_records(const char* f) {
  ...
  if (!fp) return;
  fprintf(fp,
          "记录编号\t患者编号\t医生编号\t类型\t日期\t队列号\t状态\t描述\t总费用"
          "\t医保支付\t个人支付\n");
...
#define UTF8_TITLE_DUTY_SCHEDULE "值班排班"
#define UTF8_MSG_EMPTY_SCHEDULE "当前没有符合条件的排班记录。"
...
void init_system_data() {
  ...
  // 启动自愈：自动对齐“床位账”与“住院账”，并补齐缺失床位节点
  reconcile_bed_snapshot_with_inpatients();

  ensure_duty_schedule_file();

  // 若启动时未能从文件读取到急诊手术室数据，则硬编码生成 OR-01 至 OR-03
  // 为备用抢救室
  if (or_head == NULL) {
    for (int i = 3; i >= 1; i--) {
      OperatingRoomNode* n = calloc(1, sizeof(OperatingRoomNode));
...
#define FREE_NODES(head, type) \
  ...
  FREE_NODES(nurse_head, NurseNode);
