...
void view_my_queue(DoctorNode* doctor) {
  ...
  while (curr) {
    ...
    if ((curr->type == RECORD_REGISTRATION || curr->type == RECORD_EMERGENCY) &&
        ...
        strstr(curr->description, "|CALLED") ==
            NULL &&  // 【核心修复】：只看还没被叫号的！
      ...
      char pay_status[16];
      if (curr->status == 1)
        strcpy(pay_status, "已交费");
      else
        strcpy(pay_status, "未交费");

      if (curr->type == RECORD_EMERGENCY) {
        if (curr->queue_number == 1) {
          printf(COLOR_RED
                 "[120直通置顶] 号码:%03d | 状态:大抢救 | %s | 姓名:%-8s | "
                 "编号:%s\n" COLOR_RESET,
                 curr->queue_number, pay_status, p ? p->name : "未知",
                 curr->patient_id);
        } else if (curr->queue_number > 0 && curr->queue_number < 100) {
          printf(COLOR_RED
                 "[1级濒危急诊] 号码:%03d | 状态:大抢救 | %s | 姓名:%-8s | "
                 "编号:%s\n" COLOR_RESET,
                 curr->queue_number, pay_status, p ? p->name : "未知",
                 curr->patient_id);
        } else if (curr->queue_number >= 100 && curr->queue_number < 500) {
          printf(COLOR_YELLOW
                 "[2级危重急诊] 号码:%03d | 状态:优先看 | %s | 姓名:%-8s | "
                 "编号:%s\n" COLOR_RESET,
                 curr->queue_number, pay_status, p ? p->name : "未知",
                 curr->patient_id);
        } else if (curr->queue_number >= 500 && curr->queue_number < 900) {
          printf(COLOR_CYAN
                 "[3级普通急诊] 号码:%03d | 状态:排队中 | %s | 姓名:%-8s | "
                 "编号:%s\n" COLOR_RESET,
                 curr->queue_number, pay_status, p ? p->name : "未知",
                 curr->patient_id);
        } else {
          printf(
              "[4级非急症]   号码:%03d | 状态:延后看 | %s | 姓名:%-8s | "
              "编号:%s\n",
              curr->queue_number, pay_status, p ? p->name : "未知",
              curr->patient_id);
        }
...
PatientNode* call_next_patient(DoctorNode* doctor) {
  ...
  if (!target) {
    ...
    while (curr) {
      ...
      if ((curr->type == RECORD_REGISTRATION ||
           curr->type == RECORD_EMERGENCY) &&
          strstr(curr->description, "|CALLED") == NULL &&
          strstr(curr->description, "|DONE") ==
              NULL &&  // 双重保险：坚决不叫已经看完的
          strcmp(curr->doctor_id, doctor->id) == 0 &&
          strncmp(curr->date, today, 10) == 0) {
        // 找到排队号最小的（最靠前的）作为目标
  ...
  if (target->type == RECORD_EMERGENCY) {
    if (target->queue_number == 1) {
      // 120 救护车物理插队
      printf(COLOR_RED
             "【120急救红色指令】请立即准备抢救！患者：%s（编号：%"
             "s）\n" COLOR_RESET,
             patient ? patient->name : "未知", target->patient_id);
    } else if (target->queue_number > 0 && target->queue_number < 100) {
      // 分诊 1 级 (号段 10+)
      printf(COLOR_RED
             "【1级濒危通报】患者情况极危重，请立刻抢救！患者：%s（编号：%"
             "s）\n" COLOR_RESET,
             patient ? patient->name : "未知", target->patient_id);
    } else if (target->queue_number >= 100 && target->queue_number < 500) {
      // 分诊 2 级 (号段 100+)
      printf(COLOR_YELLOW
             "【2级危重通报】患者危重，请优先予以查体！患者：%s（编号：%"
             "s）\n" COLOR_RESET,
             patient ? patient->name : "未知", target->patient_id);
    } else {
...
void prescribe_medicine_advanced(DoctorNode* doctor, PatientNode* patient) {
  ...
  printf(COLOR_CYAN "检索到药品：[%s] %s (单价: %.2f，库存: %d)\n" COLOR_RESET,
         med->id, med->common_name, med->price, med->stock);
  printf("请输入开药数量: ");
  amount = safe_get_int();
  ...
  add_new_record(patient->id, doctor->id, RECORD_CONSULTATION, desc,
                 med->price * amount, 0);

  printf(
      COLOR_GREEN
      "电子处方已开具，已发送至药房中心。患者需先缴费方可取药。\n" COLOR_RESET);
  wait_for_enter();
}
...
void generate_checkup_report(DoctorNode* doctor) {
  ...
  while (curr && count < 100) {
    if (curr->type == RECORD_CHECKUP && curr->status == 1 &&
        strcmp(curr->doctor_id, doctor->id) == 0 &&
        strncmp(curr->description, "CHECKUP|待检|", 13) == 0 &&
        strstr(curr->description, "|REPORTED") == NULL) {
  ...
  if (count == 0) {
    printf(COLOR_YELLOW "当前没有待录入结果的检查单。\n" COLOR_RESET);
    wait_for_enter();
    return;
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    printf(" %d. 患者: %-10s | 检查项目: %-20s | 流水号: %s\n", i + 1,
           p ? p->name : "未知", available[i]->description,
           available[i]->record_id);
  }
  printf(" 0. 返回\n");
  print_divider();
  printf("请选择: ");
...
void book_surgery_service(DoctorNode* doctor, PatientNode* patient) {
  ...
  if (doctor->privilege_level < requested_level) {
    printf(COLOR_RED "\n【权限不足拦截】\n" COLOR_RESET);
    printf(COLOR_YELLOW
           "当前主刀: %s (资质: Level %d) | 拟开展手术: Level %d\n",
           doctor->name, doctor->privilege_level, requested_level);
    printf(COLOR_RED
           "警告：您的临床手术权限不足，严禁越级主刀！\n" COLOR_RESET);

    // 紧急摇人机制：生命至上！
    printf(COLOR_CYAN
           "\n生命至上！是否紧急呼叫具备资质的上级专家代刀抢救？(1=呼叫, "
           "0=放弃): " COLOR_RESET);
    if (safe_get_int() != 1) {
      printf("流程已中止。\n");
      wait_for_enter();
    ...
    while (expert) {
      ...
      if (expert->privilege_level >= requested_level &&
          strcmp(expert->id, doctor->id) != 0) {
    ...
    if (expert_count == 0) {
      printf(COLOR_RED
             "极度危急：当前全院无具备该资质的专家可调度，请立即启动跨院转运预"
             "案！\n" COLOR_RESET);
      wait_for_enter();
      return;
    }
    ...
    while (1) {
      ...
      if (!chosen_expert || chosen_expert->privilege_level < requested_level ||
          strcmp(chosen_expert->id, doctor->id) == 0) {
        printf(COLOR_RED
               "【调度拦截】工号错误、该医生资质不符，或不能呼叫自己！请重新输"
               "入。\n" COLOR_RESET);
      ...
      while (temp_room) {
        if (temp_room->status == 1 &&
            strcmp(temp_room->current_doctor, chosen_expert->id) == 0) {
          is_operating = 1;
          break;
        }
        temp_room = temp_room->next;
      ...
      if (is_operating) {
        printf(COLOR_RED
               "【调度拦截】该专家目前正在手术室中主刀，无法分身！请选择列表中"
               "的空闲专家。\n" COLOR_RESET);
        continue;  // 拦截成功，让用户重新输入
      }
  ...
  if (strcmp(doctor->id, actual_surgeon_id) != 0) {
    snprintf(desc, sizeof(desc), "SURGERY|Level:%d|Room:%s|一助(申请人):%s",
             requested_level, available_room->room_id, doctor->name);
  } else {
    snprintf(desc, sizeof(desc), "SURGERY|Level:%d|Room:%s", requested_level,
  ...
  printf(COLOR_GREEN "\n手术排期成功！\n" COLOR_RESET);
  printf("分配房间：%s\n", available_room->room_id);
  printf("主刀专家：%s (工号:%s)\n", find_doctor_by_id(actual_surgeon_id)->name,
         actual_surgeon_id);
  wait_for_enter();
}
...
void view_operating_room_status(void) {
  OperatingRoomNode* curr_or = or_head;
  int or_count = 0;

  print_header("手术室实时状态看板");
  ...
  while (curr_or) {
    const char* st_text;
    const char* doctor_text = curr_or->current_doctor;

    if (curr_or->status == 0)
      st_text = COLOR_GREEN "空闲待命" COLOR_RESET;
    else
      st_text = COLOR_RED "手术中" COLOR_RESET;

    if (strcmp(curr_or->current_doctor, "N/A") != 0) {
      DoctorNode* d = find_doctor_by_id(curr_or->current_doctor);
      if (d) doctor_text = d->name;
    }

    printf("%-10s %-12s %-15s %-15s\n", curr_or->room_id, st_text, doctor_text,
           curr_or->current_patient);
...
void admission_service(DoctorNode* doctor, PatientNode* patient) {
  ...
  while (1) {
    ...
    deposit = safe_get_double_allow_negative();
  ...
  if (strcmp(doctor->id, resident_id) == 0) {
    printf(COLOR_RED
           "【医疗安全警告】主治医生与管床医生不能为同一人！必须形成上下级梯队"
           "！\n收治流程已强制中止。\n" COLOR_RESET);
    wait_for_enter();
    return;
  }
  ...
  if (res_doc->privilege_level > 2) {
    printf(COLOR_YELLOW
           "【流程建议】您指派的管床医生 (%s) 具有高级专家职称 (Level "
           "%d)。系统允许收治，请确认资源分配是否合理。\n" COLOR_RESET,
           res_doc->name, res_doc->privilege_level);
  }
...
void transfer_patient_to_icu(DoctorNode* doctor) {
  ...
  if (strcmp(patient->attending_doctor_id, doctor->id) != 0 &&
    ...
    printf(COLOR_RED
           "越权拦截：您不是该患者的主治或管床医生，无权越级操作床位流转！"
           "\n" COLOR_RESET);
    wait_for_enter();
    return;
  }
  ...
  if (!new_bed) {
    // 【ICU 资源熔断机制】
    printf(COLOR_RED
           "\n【全院红色行政告警】当前重症监护室 (ICU) "
           "床位已全部耗尽！\n" COLOR_RESET);
    printf(COLOR_YELLOW
           "系统已锁定流转。请立即联系医务处执行紧急加床或跨科室借床调度预案！"
           "\n" COLOR_RESET);
    wait_for_enter();
    return;
  }
  ...
  // 生成流转计费记录
  add_new_record(patient->id, doctor->id, RECORD_HOSPITALIZATION,
                 "SYS|病区流转:普通病房转入ICU重症监护室", 500.0, 0);
...
void discharge_inpatient_service(DoctorNode* doctor) {
  ...
  // 结转押金（保留旧余额，避免误判为覆盖）
  double refund = patient->hospital_deposit;
  double old_balance = patient->account_balance;
  patient->account_balance = old_balance + refund;
  ...
  printf(COLOR_GREEN
         "出院审批通过！已释放床位。押金退回: %.2f 元 | 余额: %.2f -> %.2f "
         "元\n" COLOR_RESET,
         refund, old_balance, patient->account_balance);
  wait_for_enter();
}
...
void view_my_inpatients(DoctorNode* doctor) {
  ...
  while (curr) {
    ...
    if (is_attending || is_resident) {
      char role_str[32] = {0};
      if (is_attending && is_resident)
        strcpy(role_str, "主治+管床");
      else if (is_attending)
        strcpy(role_str, "主治医师");
      else
        strcpy(role_str, "管床医师");

      printf("%-10s %-12s %-10s %-15s %-10.2f %-15s\n",
             curr->bed_id[0] ? curr->bed_id : "暂无", curr->id, curr->name,
             role_str, curr->hospital_deposit, curr->admission_date);
      count++;
    }
    curr = curr->next;
  }
...
void view_department_inpatients(DoctorNode* doctor) {
  ...
  while (curr) {
    ...
    if (bed && strcmp(bed->dept_id, doctor->dept_id) == 0) {
      ...
      printf("%-10s %-12s %-6s %-12s %-12s\n", curr->bed_id, curr->id,
             curr->name, att ? att->name : "N/A", res ? res->name : "N/A");
      count++;
    }
    curr = curr->next;
  }
...
void doctor_subsystem() {
  ...
  while (room) {
    if (room->status == 1 && strcmp(room->current_doctor, doctor->id) == 0) {
      ...
      if (!current_patient)
        current_patient = find_inpatient_by_id(room->current_patient);
      printf(COLOR_RED
             "\n【抢救通报】您有紧急代刀手术正在进行！系统已自动为您切入抢救状"
             "态！\n" COLOR_RESET);
      wait_for_enter();
      break;
    }
  ...
  if (!current_patient) {
    ...
    if (latest_called) {
      ...
      if (!current_patient)
        current_patient = find_inpatient_by_id(latest_called->patient_id);
    }
  }
  // =================================================================
  ...
  while (choice != 0) {
    ...
    printf("当前当值专家: %s（资质: Level %d | 科室: %s）\n", doctor->name,
           doctor->privilege_level, doctor->dept_id);
    ...
    while (temp_in) {
      if (strcmp(temp_in->attending_doctor_id, doctor->id) == 0 ||
          strcmp(temp_in->resident_doctor_id, doctor->id) == 0) {
        my_inpatient_count++;
      }
    ...
    printf(" 7. 结束当台手术\n");
    printf(" 8. 查看手术室状态看板\n");
    printf(" 9. 收治入病区 (双医生绑定制)\n");
    printf("10. 紧急转入 ICU 重症监护室\n");
    printf("11. 查看我的住院患者 (管床/主治查房)\n");
    printf("12. 查看科室在院大名单\n");
    printf("13. 个人工作量统计\n");
    printf(COLOR_CYAN
           "14. 结束当前门诊就诊 (实时落盘并清空状态)\n" COLOR_RESET);
    printf("15. 办理患者出院\n");
    printf(" 0. 退出登录\n");
    print_divider();
    printf("请选择指令: ");
    ...
    switch (choice) {
      case 1:
        view_my_queue(doctor);
        break;
      case 2:
        current_patient = call_next_patient(doctor);
        break;
      case 3:
        if (current_patient)
          prescribe_medicine_advanced(doctor, current_patient);
        else {
          printf(COLOR_YELLOW "请先呼叫患者！\n" COLOR_RESET);
          wait_for_enter();
        }
        break;
      case 4:
        if (current_patient)
          prescribe_checkup_service(doctor, current_patient);
        else {
          printf(COLOR_YELLOW "请先呼叫患者！\n" COLOR_RESET);
          wait_for_enter();
        }
        break;
      case 5:
        generate_checkup_report(doctor);
        break;
      case 6:
        if (current_patient) {
          ...
          while (temp_room) {
            if (temp_room->status == 1 &&
                strcmp(temp_room->current_patient, current_patient->id) == 0) {
              if (strcmp(temp_room->current_doctor, doctor->id) != 0) {
                printf(COLOR_CYAN
                       "\n【系统提示】患者已成功移交给上级专家接手。您的首诊任"
                       "务结束，可随时呼叫下一位患者。\n" COLOR_RESET);
      ...
      case 7:
        ...
        printf(COLOR_CYAN
               "\n【系统提示】当前手术已结束，接诊状态已清空。\n"
               "可随时呼叫下一位患者。\n" COLOR_RESET);
        break;
      case 8:
        view_operating_room_status();
        break;
      case 9:
        if (current_patient)
      ...
      case 10:
        ...
        break;
      case 11:
        view_my_inpatients(doctor);
        break;
      case 12:
        view_department_inpatients(doctor);
        break;
      case 13:
        view_doctor_statistics(doctor);
        break;
      case 14:
        if (current_patient) {
          char today_end[32];
        ...
        break;
      case 15:
        discharge_patient_service(doctor);
        break;
      case 0:
        break;
      default:
...
void discharge_patient_service(DoctorNode* doctor) {
  PatientNode* available[100];  // 假设最多100个
  int count = 0;
  int choice;
  RecordNode* curr = record_head;
  double total_cost = 0;
  ...
  // 收集可出院的患者（当前医生负责的住院患者）
  PatientNode* patient = inpatient_head;
  while (patient && count < 100) {
    if ((strcmp(patient->attending_doctor_id, doctor->id) == 0 ||
         strcmp(patient->resident_doctor_id, doctor->id) == 0)) {
      available[count++] = patient;
    }
    patient = patient->next;
  }

  if (count == 0) {
    printf(COLOR_YELLOW "当前没有您负责的住院患者。\n" COLOR_RESET);
    wait_for_enter();
    return;
  }
  ...
  print_divider();
  for (int i = 0; i < count; i++) {
    printf(" %d. 患者: %-10s | 编号: %-12s | 床位: %-10s | 入院日期: %s\n",
           i + 1, available[i]->name, available[i]->id,
           available[i]->bed_id[0] ? available[i]->bed_id : "暂无",
           available[i]->admission_date);
  }
  printf(" 0. 返回\n");
  print_divider();
  printf("请选择: ");
  ...
  printf(COLOR_GREEN
         "出院办理成功！已释放床位。押金退回: %.2f 元 | 余额: %.2f -> %.2f "
         "元\n" COLOR_RESET,
         refund, old_balance, patient->account_balance);
  wait_for_enter();
}