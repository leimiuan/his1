...
// 核心职责：
// 1. 拦截所有非法、越界、溢出的用户输入，防止系统宕机或被恶意注入 (Buffer Overflow)；
// 2. 统一全院 UI 渲染风格，提供标准化的控制台绘制接口；
// 3. 提供高精度的时间戳生成，为病历建档、财务对账、以及 120 急救出勤提供绝对的时间准绳。
// 
...
int safe_get_int() {
    ...
    while (1) {
        ...
        // 2. !has_trailing_non_space : 确保数字后面没有跟着乱码 (例如输入 "123x" 必须被拒绝)
...
double safe_get_double() {
    ...
    while (1) {
        ...
        if (end_ptr != input && !has_trailing_non_space(end_ptr) && errno != ERANGE) {
            ...
            // 【系统级绝对指令拦截】：根据国家医疗财务规范，门诊/住院单笔发生金额最高不得超过 10 万元！
            if (value > 100000.0) {
                printf(COLOR_RED " 拦截：金额超限！根据医院财务规范，单笔发生金额最高不得超过 100,000 元。\n 请重新输入: " COLOR_RESET);
                continue; // 金额过大，打回重输
            }
            // 防御恶意套现或系统 BUG：任何资金流转数据绝对不允许为负数
            if (value < 0.0) {
                printf(COLOR_RED " 拦截：金额异常！涉及资金流转的数据绝对不允许为负数。\n 请重新输入: " COLOR_RESET);
                continue; // 负数，打回重输
            }
        ...
        printf(COLOR_RED " 无效金额！请输入一个合法的金额数值 (例如 12.50): " COLOR_RESET);
    }
}
...
void wait_for_enter() {
    char dummy[8];
    printf(COLOR_CYAN "\n请按回车键继续..." COLOR_RESET);
    read_input_line(dummy, sizeof(dummy)); // 读取并丢弃任何输入，直到用户真切地按下回车
}
...
void show_progress_bar(const char* task_name) {
    ...
    for (int i = 0; i <= total; i++) {
        ...
        fflush(stdout); // 强制立刻刷新输出流，确保 '#' 是逐个显示，而不是积攒后一次性吐出
        
        // 简单的底层 CPU 循环延时器
        // volatile 关键字在这里至关重要：明确警告现代 C 编译器(如 GCC/Clang)绝对不要“智能优化”掉这个看似无用的空循环
 ...
 * 格式被严格限制为 YYYY-MM-DD，用于生成所有常规诊疗单据 (门诊挂号、取药) 的基础时间戳。
 */
 ...
 * @param time_buffer 外部传入的缓冲区，用于接收生成的字符串 (建议最少分配 32 字节)
