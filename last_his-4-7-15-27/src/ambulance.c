...
static const char* map_level_to_title(int level) {
    switch (level) {
        case 4: return "正主任医师 (Lv.4)";
        case 3: return "副主任医师 (Lv.3)";
        case 2: return "主治医师 (Lv.2)";
        case 1: return "住院医师 (Lv.1)";
        default: return "未知资历";
    }
}
...
static int verify_doctor_authority_amb(const char* doctor_id, int req_level) {
    ...
    if (doc->privilege_level < req_level) {
        printf(COLOR_RED "[安全警报] 资质不符！急救任务要求: %s，当前指派医生: %s\n" COLOR_RESET, 
               map_level_to_title(req_level), map_level_to_title(doc->privilege_level));
        return 0;
    }
    return 1;
}
        ...
        switch (choice) {
            ...
            case 3: {
                ...
                if (target && target->status == AMB_DISPATCHED) {
                    ...
                    printf(COLOR_GREEN ">>> 车辆 %s 已确认回场，并已执行深度洗消程序，转为待命状态。\n" COLOR_RESET, target->vehicle_id);
                    ...
                    if (has_patient == 1) {
                        char p_id[MAX_ID_LEN], d_id[MAX_ID_LEN], intern_id[MAX_ID_LEN], date[32], desc[MAX_DESC_LEN];
                        int req_level;
                        
                        printf(COLOR_CYAN "==== 启动 120 绿色直通建档程序 (自动强制最高优先级) ====\n" COLOR_RESET);
                        
                        printf("患者编号(或暂定急救号): "); safe_get_string(p_id, MAX_ID_LEN);
                        printf("当前日期(YYYY-MM-DD): "); safe_get_string(date, 32);
                        ...
                        // ====== 【核心修复：智能防撞扫描大名单】 ======
                        printf(COLOR_CYAN "\n正在全院检索满足 Level %d 抢救资质的专家大名单...\n" COLOR_RESET, req_level);
                        print_divider();
                        ...
                        while (expert) {
                            if (expert->privilege_level >= req_level) {
                                ...
                                if (!is_operating) {
                                    ...
                                    if (on_duty) printf(COLOR_GREEN "[在院当值] " COLOR_RESET);
                                    else printf(COLOR_YELLOW "[二线听班] " COLOR_RESET);
                                    
                                    printf("专家: %-8s | 工号: %-6s | 资质: Level %d | 科室: %s\n", 
                        ...
                        if (expert_count == 0) {
                            printf(COLOR_RED "【全院红色告警】当前全院无任何满足该资质且空闲的医生！请启动跨院转运预案！\n" COLOR_RESET);
                            wait_for_enter();
                            break; 
                        }
                        ...
                        while (1) {
                            ...
                            safe_get_string(d_id, MAX_ID_LEN); // 直接存入后续建档需要的 d_id 中！
                            
                            // 校验 1：权限与存在性校验
                            if (!verify_doctor_authority_amb(d_id, req_level)) {
                                printf(COLOR_RED "[拦截] 医生工号无效或资质不符，请重新输入！\n" COLOR_RESET);
                                continue; 
                            }
                            ...
                            OperatingRoomNode* temp_room = or_head;
                            while (temp_room) {
                                if (temp_room->status == 1 && strcmp(temp_room->current_doctor, d_id) == 0) {
                                    is_operating = 1; break;
                                }
                                temp_room = temp_room->next;
                            }
                            if (is_operating) {
                                printf(COLOR_RED "[拦截] 该医生正在抢救其他患者，分身乏术！请重新选择。\n" COLOR_RESET);
                                continue; 
                            }
                        ...
                        if (!find_doctor_by_id(intern_id)) {
                            printf(COLOR_RED "直通建档失败：未指定有效的管床医生。\n" COLOR_RESET);
                            wait_for_enter();
                            break;
                        }
                        ...
                        snprintf(auto_desc, sizeof(auto_desc), "[救护车直通入抢救室] %s", desc);

                        // 默认记录为类型 4 (住院/手术)，金额暂估 0.0，排队号强制为 1
                        add_new_record(p_id, d_id, RECORD_HOSPITALIZATION, auto_desc, 0.0, 1);
                        ...
                        if (avail_room) {
                            ...
                            printf(COLOR_RED "【物理资源调度】已自动为您抢占并锁定抢救室: %s\n" COLOR_RESET, avail_room->room_id);
                        } else {
                            printf(COLOR_YELLOW "【物理资源告警】当前全院无空闲手术室/抢救室！已通知急诊科就地开展抢救！\n" COLOR_RESET);
                        }
                        // ================================================================

                        printf(COLOR_GREEN "急救通道已激活！患者记录已下发，主刀[%s] & 管床[%s] 已接收到强提醒任务。\n" COLOR_RESET, d_id, intern_id);
                    }
                } else {
                    printf(COLOR_RED "操作失败：未找到该车，或车辆当前未处于执行任务状态。\n" COLOR_RESET);
                }
                wait_for_enter();
                break;
            }
