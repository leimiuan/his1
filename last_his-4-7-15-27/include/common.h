// [核心架构说明]：
// 本文件为 HIS 系统全局公共组件与核心实体类定义层 (Common Entity & Definitions)。
...
// 【最新架构演进】：在 PatientNode 中新增双医生责任制字段，彻底打通临床病区管床逻辑。
...
typedef enum {
    ...
    RECORD_EMERGENCY = 5,        // 急诊绿色通道 (高优先级，120救护车拉回时系统自动生成此单据)
...
typedef struct PatientNode {
    ...
    double hospital_deposit;  // 住院预缴押金 (合规风控线：须为100倍数且 >= 200*天数)

    // [核心架构新增]：支撑住院病区“双医生责任制” (临床合规要求)
    char attending_doctor_id[MAX_ID_LEN]; // 主治医生工号 (专家级，负责审批诊疗方案与手术把关)
    char resident_doctor_id[MAX_ID_LEN];  // 管床医生工号 (主治/住院级，负责日常查房与病程记录)
...
typedef struct RecordNode {
    ...
    // [架构联动关键]：挂号排队序位。
    // 在普通模式下为递增排号。若被 120 急救直通通道触发，会被系统强制覆写为 1，实现物理插队置顶。
    int queue_number;         
...
// 定义兼容性互斥锁，告知所有包含本头文件的 .c 文件：节点已在全局基石中定义完成，无需重复定义！
...
typedef struct OperatingRoomNode {
    ...
    int status;                 // 三态机 - 0: 空闲可用, 1: 正在被高难手术占用, 2: 清场深度消毒中
...
extern PatientNode* inpatient_head;  // 住院患者大名单 (门诊转住院时涉及内存地址平滑搬家)
